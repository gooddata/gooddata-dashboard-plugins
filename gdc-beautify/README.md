...
2.  Develop your plugin code in `src/dp_gdc_beautify`

    The `src/dp_gdc_beautify/Plugin.tsx` is the main plugin file where you have to register all
    your custom content. However, you can create as many new files as you want under the `src/dp_gdc_beautify`
...
5.  Add plugin to one or more workspaces: `npm run add-plugin -- <url>`
    ...
    Run the `npm run add-plugin -- "https://your.hosting/pluginDirOfYourChoice/dp_dp_gdc_beautify.js"` to
...
6.  Use plugin on a dashboard: `npm run link-plugin -- <plugin-object-id>`
    ...
    If your plugin supports parameterization (see [src/dp_gdc_beautify](./src/dp_gdc_beautify/Plugin.tsx)) and
...
This project is setup so that all your custom code must be self-contained in the [src/dp_gdc_beautify](./src/dp_gdc_beautify) directory.

The [src/dp_gdc_beautify_engine](./src/dp_gdc_beautify_engine) and [src/dp_gdc_beautify_entry](./src/dp_gdc_beautify_entry) directories contain essential plugin boilerplate.
