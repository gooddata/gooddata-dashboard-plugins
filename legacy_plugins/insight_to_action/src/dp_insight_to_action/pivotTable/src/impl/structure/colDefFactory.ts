    ...
    for (const col of cols) {
        switch (col.type) {
            ...
            case "seriesCol": {
                ...
                const cellRendererProp = !state.cellRendererPlaced ? { cellRenderer: "loadingRenderer" } : {};
                const colDef: ColDef = {
                    type: col.seriesDescriptor.measureDefinition?.customColumn
                        ? CUSTOM_COLUMN
                        : MEASURE_COLUMN,
                    colId: col.id,
                    field: col.id,
                    // headerName: col.seriesDescriptor.measureDefinition.measure.localIdentifier === 'customColumn' ? 'Send' : col.seriesDescriptor.measureTitle(),
